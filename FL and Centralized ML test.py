...
if __name__ == "__main__":
    ...
    #Create an empty file, it also empties the file if it already existed
    results_dictionary = {}
    for metric in results_columns_names:
        results_dictionary[metric]=[]
    df = pd.DataFrame(results_dictionary)
    df.to_csv(results_file, index=False,quoting=csv.QUOTE_NONNUMERIC)
    
    
    
    number_of_experiments=3
    for number_of_clients in [5,7,10,13,15,17,20]:#A list of the number of clients to use in the multiple tests
